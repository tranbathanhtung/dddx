    ...
    >
      {isImageOnly ? (
        canPreview ? (
          <button
            type="button"
            className="size-8 overflow-hidden shrink-0 rounded-[calc(var(--an-input-border-radius)-var(--an-context-padding)-2px)] cursor-pointer border-0 p-0 bg-transparent"
        ...
        <>
          {isImage && url ? (
            canPreview ? (
              <button
                type="button"
                className="w-8 self-stretch overflow-hidden shrink-0 rounded-[calc(var(--an-input-border-radius)-var(--an-context-padding)-2px)] cursor-pointer border-0 p-0 bg-transparent"
                aria-label={`Preview ${filename}`}
                onClick={openLightbox}
              >
                <img
