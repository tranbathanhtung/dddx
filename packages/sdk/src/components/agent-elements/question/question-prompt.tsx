...
export type QuestionPromptProps = {
  ...
  skipLabel?: string;
  allowSkip?: boolean;
  /** Blocks Continue/Skip while the agent is still streaming or tool input is incomplete. */
...
export function QuestionPrompt({
  ...
  const handleSubmit = () => {
    if (submitDisabled || !canSubmit || !activeQuestion) return;
    if (activeQuestion.kind === "text") {
  ...
  return (
    <div className={cn("px-3 py-2 space-y-2 bg-background", className)}>
      ...
      {activeQuestion.kind !== "text" &&
              ...
              return (
                ...
                >
                  ...
                  <span className="text-sm text-an-tool-color">
                    {option.label?.trim() || option.id || `Option ${idx + 1}`}
                    {option.description && (
        ...
        <p className="text-xs text-an-tool-color-muted px-0.5">
          No choices were provided — use Skip or type your reply in the chat
          box below.
        </p>
      ) : null}
        ...
        <div className="flex items-center justify-end gap-1.5">
          {allowSkip && (
            <button
              ...
              className="h-6 px-2 rounded-[4px] text-sm text-muted-foreground hover:text-an-tool-color hover:bg-muted/50 active:scale-[0.98] transition-[background-color,color,transform] duration-150 disabled:opacity-60 disabled:pointer-events-none"
            >
              {skipLabel}
            </button>
          )}
