...
import {
  getResolvedQuestionToolSummary,
  isQuestionToolPartResolved,
} from "./question-part-status";
import { parseQuestionToolInput } from "./questions-v2";
...
export type QuestionToolPart = {
  ...
  output?: {
    ...
    output?: string;
    metadata?: { answers?: unknown[]; truncated?: boolean };
  };
};
...
export function QuestionTool({ part }: QuestionToolProps) {
  ...
  const questions: QuestionConfig[] = parseQuestionToolInput(part.input);
  ...
  const remoteResolved = isQuestionToolPartResolved(part);
  ...
  const summaryText = useMemo(() => {
    ...
    if (remoteResolved) {
      return getResolvedQuestionToolSummary(part, questions);
    }
